...
if data_f is not None:
    data_f = data_f[:args.num_objects]
...
if args.targeted:
    res = adversarial_utils.targeted_attack(args.checkpoint, args.output, x_pl, t_pl, model_loss_fn, data_x, data_t, class_names, data_f = data_f, restrict = args.restrict, iter = args.iter, eps_list = args.eps, momentum = args.momentum, one_hot = False, extra_feed_dict = {is_training: False})
    if data_f is None:
    ...
    res = adversarial_utils.untargeted_attack(args.checkpoint, args.output, x_pl, t_pl, model_loss_fn, data_x, data_t, class_names, data_f = data_f, restrict = args.restrict, iter = args.iter, eps_list = args.eps, momentum = args.momentum, one_hot = False, extra_feed_dict = {is_training: False})
    if data_f is None:
